...
var PagerView = function(id){
	...
	this.render = function(){
		...
		str += ' 共' + self._pageCount + '页, ' + self.itemCount + '条 ';
		str += '</div><!-- /.pagerView -->\n';
	...
	for ( var i = 0; i < tr.length; i++) { 
		if (i % 2 == 0) {
			...
			tr[i].onmouseover = function() {
				this.style.backgroundColor = "#B7E1F6";
			};
...
function swnidow(iWidth,iHeight,url){ 
	var iTop=(window.screen.availHeight - 20 - iHeight) /2;
	var iLeft = (window.screen.availWidth - 10 - iWidth) /2;
	return	window.showModalDialog(url,window,"dialogWidth:"+iWidth+"px;dialogHeight:"+iHeight+"px;dialogTop:"+iTop+"px;dialogLeft:"+iLeft+"px;edge: Raised; center: Yes; help: No; resizable: No;status:no;location:no;");
} 
 
function lodopPContent(htmls,ver){ 
	var printWindow = window.open('about:blank', '', 'left=0,top=0,width=1000,height=700'); 
	printWindow.document.write('<body><div id="printval">' + htmls + "</div></body>");
	printWindow.document.close();
	printWindow.focus();
	LODOP=getLodop();
	LODOP.SET_PRINT_PAGESIZE(ver, 0, 0, "A4") ; //A4  打印   ver =1纵向  ver =横向 
	LODOP.ADD_PRINT_HTM(30,30,"90%","85%",printWindow.document.getElementById("printval").innerHTML);
	LODOP.PREVIEW();
	printWindow.close();
}
