...
public class Words {
	...
	public boolean insertAtMark(String mark, String newText) { 
		Dispatch bookMarks = Dispatch.call(document, "Bookmarks").toDispatch();
        boolean bookMarkExist = Dispatch.call(bookMarks, "Exists", mark).toBoolean();
        if (bookMarkExist) { 
            Dispatch rangeItem = Dispatch.call(bookMarks, "Item", mark).toDispatch();
            Dispatch range = Dispatch.call(rangeItem, "Range").toDispatch();
            Dispatch.put(range, "Text", new Variant(newText));
            return true;
        } 
        return false;
	} 
